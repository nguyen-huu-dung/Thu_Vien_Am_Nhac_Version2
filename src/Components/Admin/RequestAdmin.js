...
class RequestAdmin extends HTMLElement {
    constructor() {
        super();
    ...
    connectedCallback() {
        ...
        <div id="admin-request">
            <div class="admin-content">
                <div class="admin-name">Quản lý yêu cầu bài hát</div>
                <add-form></add-form>
                <div class="admin-list">
                    <table>
                        <tr>
                            <th class="col-3">Gmail</th>
                            <th class="col-6">Tên bài hát</th>
        ...
        getData('requests').then((res) => {
            const listRequest = res;
            printList(listRequest);

        })

        // Print list
        const printList = (listRequest) => {
            removeList();
            ...
            let index = 1;
            let i;
            const max = listRequest.length;
            for (i = index - 1; i < index + 9; ++i) {
                this.shadow.querySelector('table').insertAdjacentHTML('beforeend', `
            <tr>
                <td>${listRequest[i].email}</span></td>
                <td>${listRequest[i].name} - <span>${listRequest[i].singer}</span></td>
                <td>
...
window.customElements.define('request-admin', RequestAdmin);